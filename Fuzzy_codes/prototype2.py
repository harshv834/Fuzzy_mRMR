...
def main():
	...
	acc1 = np.zeros((k_max,1));
	acc2 = np.zeros((k_max,1));
	acc_ts = np.zeros((k_max,1));
	acc1_ts = np.zeros((k_max,1));
	acc2_ts = np.zeros((k_max,1));
	avg_feature = np.zeros((k_max,1));
	redundant_features = np.zeros((k_max,1));
	...
	for k in range(k_max):
		...
		for train, test in ss.split(X,y):
			...
			Scores, features = select(Xtr, ytr, c_labels,n_clus,(k+1)*5);
			Xtrnew = Xtr[:,features];
			Xtsnew = Xts[:,features];
			for l in range(Xtrnew.shape[1]):
				for m in range(l):
					if(midd(Xtrnew[:,l],Xtrnew[:,m])>1):
						redundant_features[k]=redundant_features[k]+1;			
			...
			acc[k] = acc[k] + metrics.accuracy_score(ytr,ynew);
			acc1[k] = acc1[k] + metrics.accuracy_score(ytr,y1);
			acc2[k] = acc2[k] + metrics.accuracy_score(ytr,y2);
			acc_ts[k] = acc_ts[k] + metrics.accuracy_score(ysnew,yts);
			acc1_ts[k] = acc1_ts[k] + metrics.accuracy_score(y1s,yts);
			acc2_ts[k] = acc2_ts[k] + metrics.accuracy_score(y2s,yts);
			j = j +1;
